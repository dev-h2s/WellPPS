...
@Entity
@Getter
public class WellFileStorageEntityBackup {
    ...
    private WellFakeRegistrationFileStorageEntityBackup fakeRegistrationFIleStorage;

    @OneToOne(fetch = LAZY) // 첨부파일과 가상계좌파일 간의 연결
    private WellVirtualAccountFileStorageEntityBackup virtualAccountFIleStorage;

    @OneToOne(fetch = LAZY) // 첨부파일과 거래처파일 간의 연결
    private WellPartnerFileStorageEntityBackup PartnerFileStorage;
